...
一个本地优先的 IELTS 阅读练习工作台。v0.8 同时提供浏览器版与 Windows 桌面版，并加入签名应用更新、可选端到端加密同步和显式信任的内容分发。
...
├── domain/         # 题型、评分、冲突合并、保险库、内容包与目录签名
├── platform/       # 浏览器、Tauri、updater、同步与内容源传输适配器
├── storage/        # v1-v3 迁移、v4 仓储、同步设置与设备信任库
...
v0.1 至 v0.8 完成情况见 [ROADMAP.md](docs/ROADMAP.md)。托管账号服务、商业 Authenticode 证书、听力、词汇 SRS、写作 AI 评分、口语 AI 评分和付费能力不在本里程碑内。
