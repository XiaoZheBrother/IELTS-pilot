...
v0.9.7 让 IELTS Pilot 从“展示诊断和建议”进入完整学习闭环：判断必须有本地证据，建议可以直接启动练习，练习完成后自动核对结果并生成下一轮任务；AI 对话支持真实流式输出，但只有通过结构与证据校验的最终回答才会进入历史。桌面版同时会明确显示更新检查是否真正完成，并保留底层错误摘要用于排障。
