...
import type { AuthorPackageDraft, PracticeSet, ReadingQuestion } from '../domain/models'
...
let draftId = `author-${Date.now()}`
const savedDrafts = ref<AuthorPackageDraft[]>(repository.listAuthorDrafts())
...
function removeSection(sectionIndex: number): void { if (activeSet().passage.sections.length > 1) activeSet().passage.sections.splice(sectionIndex, 1) }
function removeParagraph(sectionIndex: number, paragraphIndex: number): void { const paragraphs = activeSet().passage.sections[sectionIndex]!.paragraphs; if (paragraphs.length > 1) paragraphs.splice(paragraphIndex, 1) }
...
function saveDraft(): void {
  ...
  savedDrafts.value = repository.listAuthorDrafts()
  feedback.value = '草稿已保存到当前设备。'
}
function loadDraft(draft: AuthorPackageDraft): void {
  const result = validateContentPackage(draft.package)
  if (!result.ok) { feedback.value = `草稿无法打开：${result.errors.join('；')}`; return }
  draftId = draft.id
  Object.assign(content, JSON.parse(JSON.stringify(result.value)) as ContentPackageV2)
  activeSetIndex.value = 0
  feedback.value = `已打开草稿“${draft.name}”。`
}
function deleteDraft(id: string): void {
  repository.removeAuthorDraft(id)
  savedDrafts.value = repository.listAuthorDrafts()
  if (draftId === id) draftId = `author-${Date.now()}`
  feedback.value = '草稿已从当前设备删除。'
}
...
<template>
  <main class="package-editor-page page-shell">
    ...
    <section v-if="savedDrafts.length" class="author-drafts"><header><p class="section-kicker">Saved locally</p><h2>本地草稿</h2></header><article v-for="draft in savedDrafts" :key="draft.id"><div><strong>{{ draft.name }}</strong><time>{{ new Date(draft.updatedAt).toLocaleString('zh-CN') }}</time></div><button data-testid="load-author-draft" type="button" @click="loadDraft(draft)">打开</button><button data-testid="delete-author-draft" type="button" @click="deleteDraft(draft.id)">删除</button></article></section>
    ...
    <section class="editor-panel"><header><div><p class="section-kicker">Practice set</p><h2>练习与文章</h2></div><button :disabled="content.sets.length === 1" @click="removeSet">删除当前练习</button></header><div class="editor-grid">
      ...
      <label><span>栏目眉题</span><input v-model.trim="activeSet().eyebrow" /></label><label><span>等级标签</span><input v-model.trim="activeSet().level" placeholder="B2-C1" /></label>
      ...
      <label><span>建议 Band</span><input v-model.number="activeSet().estimatedBand" type="number" min="0" max="9" step="0.5" /></label><label class="editor-span"><span>练习简介</span><textarea v-model.trim="activeSet().summary" rows="2" /></label>
      <label class="editor-span"><span>主题（逗号分隔）</span><input :value="topicsText()" @input="setTopics(($event.target as HTMLInputElement).value)" /></label>
      <label><span>内容来源类型</span><select v-model="activeSet().provenance.kind"><option value="original">原创</option><option value="public-domain">公共领域</option><option value="licensed">已获授权</option></select></label><label><span>文章作者 / 权利人</span><input v-model.trim="activeSet().provenance.author" /></label>
      <label><span>文章许可协议</span><input v-model.trim="activeSet().provenance.license" /></label><label><span>文章来源链接（可选）</span><input v-model.trim="activeSet().provenance.sourceUrl" type="url" /></label>
      <label class="editor-span"><span>文章授权与来源说明</span><textarea v-model.trim="activeSet().provenance.note" rows="2" /></label>
      <label class="editor-span"><span>文章标题</span><input v-model.trim="activeSet().passage.title" /></label><label class="editor-span"><span>文章导语</span><textarea v-model.trim="activeSet().passage.deck" rows="2" /></label>
    </div>
    <div class="section-editors"><article v-for="(section, sectionIndex) in activeSet().passage.sections" :key="sectionIndex" class="section-editor"><header><strong>SECTION {{ sectionIndex + 1 }}</strong><input v-model.trim="section.heading" aria-label="章节标题" /><button type="button" :disabled="activeSet().passage.sections.length === 1" @click="removeSection(sectionIndex)">删除章节</button></header><label v-for="(_, paragraphIndex) in section.paragraphs" :key="paragraphIndex"><span>段落 {{ paragraphIndex + 1 }}</span><textarea v-model.trim="section.paragraphs[paragraphIndex]" rows="5" /><button type="button" :disabled="section.paragraphs.length === 1" @click="removeParagraph(sectionIndex, paragraphIndex)">删除段落</button></label><button type="button" @click="addParagraph(sectionIndex)">＋ 添加段落</button></article><button type="button" class="add-section" @click="addSection">＋ 添加章节</button></div>
    </section>
